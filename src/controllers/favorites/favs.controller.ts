import { BadRequestException, Body, Controller, Delete, Get, HttpCode, HttpStatus, NotFoundException, Param, Post, Put, UnprocessableEntityException, UsePipes, ValidationPipe } from "@nestjs/common";
...
export class FavoritesController {
    ...
    insertToFav(id, data_table) {
        ...
        if (!artist) {
            throw new UnprocessableEntityException(`Data with ${id} inside ${data_table} not found!`);
        }
    ...
    deleteFromFav(id, data_table) {
        ...
        if (!artist) {
            throw new NotFoundException(`Data with ${id} inside ${data_table} not found!`);
        }
