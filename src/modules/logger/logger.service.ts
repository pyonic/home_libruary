...
export class CustomLoggerService implements LoggerService {
    private readonly levels_transformer: Object = {
        0: "log",
        1: "error",
        2: "warn",
        3: "debug",
    ...
    async logRequest(req: any, res: any, meta: any = { error: false }) {
        ...
        if (meta.error) {
            ...
            res.on("finish", async () => {
                ...
                let timeout: number = new Date(finish - start).getMilliseconds();
    ...
    async saveLog(data, meta = { error: false }) {
        ...
        const sortedFiles: Array<any> = fileStats.sort((a: any, b: any) => a.size - b.size);

        if (sortedFiles.length) {
            const freshLogFile: any = sortedFiles[0];

            if (logRotationSize * 1024 < freshLogFile.size) {
                const fileName: string = `logs_${Date.now()}.log`;
    ...
    async catchUncaughtExceptions() {
        ...
        process.on('uncaughtException', async (error: Error, origin) => {
            await this.error(`Unhandled error detected: ${error.stack || error.message}`);
            if (+process.env.KILL_ON_UNHANDLED === 1) {
                process.exit(0);
            }
          });
