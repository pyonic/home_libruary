...
export class AuthService {
  ...
  async getTokens(payload) {
    const accessToken = this.generateJWT(payload, JWT_SECRET, {
      expiresIn: JWT_LIFETIME,
    });
    const refreshToken = this.generateJWT(payload, JWT_REFRESH_SECRET, {
  ...
  async refreshToken(body) {
    ...
    const payload: any = await this.verifyToken(
      refreshToken,
      JWT_REFRESH_SECRET,
    );

    const data = await this.refreshTokensRepository.findOne({
      where: { token: refreshToken },
    });

    if (data && data.login == payload.login) {
